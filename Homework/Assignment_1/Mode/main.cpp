...
int *mode(const int *array,int arySize){
    ...
    for(int x=0;x<arySize-1;x++){
        for(int y=x+1;y<arySize;y++){
            cout<<"Arry["<<x<<"] == "<<"Arry["<<y<<endl;
            if(ary[x]==ary[y]){
                cout<<"yes "<<ary[x]<<" = "<<ary[y]<<endl;
                *(ary + x) = *(ary + x) + 1;
            }
            else
                cout<<"No "<<ary[x]<<" != "<<ary[y]<<endl;
                
            }
    }
    
    int position = 0, freq = *ary;
    //Loop that checks the maximum from the short array
	for(int s = 1; s < arySize; s++)
	{
		if(freq < *(ary + s))
		{
			freq = *(ary + s);
    ...
    modeAry[0]=ary[position];//Stub returns no modes
    modeAry[1]=freq;//Stub returns Frequency 1
    
    
    //Fill the mode array
    for(int i=2;i<nmodes+2;i++){
        modeAry[i]=i;
    }
