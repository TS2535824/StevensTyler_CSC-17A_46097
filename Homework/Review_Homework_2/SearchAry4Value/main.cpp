...
//******************************************************************************
//definition of function srch1.This function is a linear search that looks for *
//a single match from two arrays.                                                     *
//******************************************************************************
int  srch1(const char sntnce[],const char pattern[],int start){
    
    int senLen=strlen(sntnce),//sentence length
        patLen=strlen(pattern); //pattern length

    int i,j;//for loop positions

    //loop through array
    for (i=start;i<(senLen-patLen+1);++i) {
...
//************************************************************************
//definition of function srchAll. This function searches for all matches *
//from two arrays of characters                                          *
//************************************************************************
void srchAll(const char sntnce[],const char pattern[],int match[]){
    
    // searches for all patterns in sentence one by one

    int k=0,index=0;

    while ((index=srch1(sntnce,pattern,index))!=-1){
...
//*****************************************************************************
//definition of function print. This function prints out contents of an array *
//of characters.                                                              *
//*****************************************************************************
...
void print(const int array[]){
   ...
   //if this first position is zero then there are no matches 
    if(array[0]==-1){
        cout<<"None"<<endl;
        return;
    }
    
    //display all positions that are not -1
